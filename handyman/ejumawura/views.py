...
from ejumawura.models import UserProfile,Category, Post, Community
from django.shortcuts import render, get_object_or_404

from ejumawura.forms import UserRegistration, UpdateProfile, UpdateProfileMeta, UpdateProfileAvatar, SaveCategory, SavePost, AddAvatar 
...
def save_post(request):
    ...
    if form.is_valid():
        form.save()
        resp['status'] = 'success'
        messages.success(request, 'Post has been saved successfully. Now allow us some time to verify your details. you might receive a call from our agent in your community for proper verification. your post will be avalable for our site visitors, as soon us we are certify with your details, and same will be communicated via whatsapp or text message')
    else:
        for field in form:
            for error in field.errors:
                resp['msg'] += str(error + '<br>')
...
def community_handymen(request, community_id):
    # Get the community by ID
    community = get_object_or_404(Community, id=community_id)
    # Get all categories that belong to this community
    categories = Category.objects.filter(community=community)
    return render(request, 'category_list.html', {'community': community, 'categories': categories})
    
    
def choose_community(request):
    communities = Community.objects.all()
    categories = Category.objects.all()
    return render(request, 'choose_community.html', {'communities': communities, 'categories': categories})


            
    
def category_posts(request, community_id, category_name):
    # Get the community and category by their names/IDs
    community = get_object_or_404(Community, id=community_id)
    category = get_object_or_404(Category, name=category_name, community=community)
    # Get all posts that belong to this category and community
    posts = Post.objects.filter(community=community, category=category, status=1)
    # Pass the community and category along with posts to the template
    return render(request, 'handymen_list.html', {'community': community, 'category': category, 'posts': posts})
        
    
def your_view_function(request, community_id, category_name):
    community = get_object_or_404(Community, id=community_id)
    category = get_object_or_404(Category, name=category_name, community=community)
    posts = Post.objects.filter(community=community, category=category, status=1)
    return render(request, 'posts_by_community_category.html', {'community': community, 'category': category, 'posts': posts})
